⚠️  DANGEROUS COMMAND: Security scan — [HIGH] Pipe to interpreter: curl | python3: Command pipes output from 'curl' directly to interpreter 'python3'. Downloaded content will be executed without inspection.
  Safer: tirith run https://registry.npmjs.org/pdf-mcp  — or: vet https://registry.npmjs.org/pdf-mcp  (https://getvet.sh)
      curl -L --max-time 20 -s https://registry.npmjs.org/pdf-mcp | python3 - <<'PY'
import sys, json
try:
...
PY
  ...
  ⚠️  DANGEROUS COMMAND: Security scan — [HIGH] Pipe to interpreter: curl | python3: Command pipes output from 'curl' directly to interpreter 'python3'. Downloaded content will be executed without inspection.
  Safer: tirith run https://api.github.com/search/repositories?q=pdf-mcp  — or: vet https://api.github.com/search/repositories?q=pdf-mcp  (https://getvet.sh)
      curl -L --max-time 20 -s 'https://api.github.com/search/repositories?q=pdf-mcp' | python3 - <<'PY'
import sys, json
