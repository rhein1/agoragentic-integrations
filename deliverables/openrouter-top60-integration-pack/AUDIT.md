...
Ranking snapshot date: **2026-08-09**
Repository baseline reconciled: `rhein1/agoragentic-integrations@27aa21c613ee6ad5ae69e49659e3fd75ba2eccad` on **2026-08-25**
...
- `covered_existing`: **5**
- `blocked_pending_qualified_host_enforcement`: **12**
- `direct_adapter`: **2**
...
| 2 | Claude Code | 7.53T | `covered_existing` | `inbound_host` | Retain the existing plugin and skill metadata; cross-link OpenRouter model routing without claiming a live Agoragentic MCP relay. |
| 3 | Kilo Code | 7.33T | `blocked_pending_qualified_host_enforcement` | `inbound_host` | No runnable Agoragentic MCP relay is available; use a supported SDK/REST match adapter if the host can embed code, otherwise remain blocked pending qualified host enforcement. |
| 4 | OpenClaw | 4.47T | `blocked_pending_qualified_host_enforcement` | `inbound_host` | No runnable Agoragentic MCP relay is available; use a supported SDK/REST match adapter if the host can embed code, otherwise remain blocked pending qualified host enforcement. |
| 5 | Cline | 3.55T | `covered_existing` | `inbound_host` | Retain the existing protocol/reference metadata only; use SDK/REST tooling outside MCP when embedding is available, with remote MCP blocked pending qualified host enforcement. |
| 6 | pi | 2.61T | `blocked_pending_qualified_host_enforcement` | `inbound_host` | No runnable Agoragentic MCP relay is available; use a supported SDK/REST match adapter if the host can embed code, otherwise remain blocked pending qualified host enforcement. |
| 7 | Descript | 1.76T | `composition_recipe` | `outbound_mcp_service` | Keep Descript's MCP service separate; implement the Agoragentic leg with a supported SDK/REST match adapter, not an Agoragentic MCP server. |
| 8 | Command Code | 1.62T | `blocked_pending_qualified_host_enforcement` | `inbound_host` | No runnable Agoragentic MCP relay is available; use a supported SDK/REST match adapter if the host can embed code, otherwise remain blocked pending qualified host enforcement. |
...
| 12 | Codex | 948B | `blocked_pending_qualified_host_enforcement` | `inbound_host` | Use the official OpenRouter SDK skill and the supported Agoragentic SDK/REST match path; no runnable Agoragentic MCP relay is available pending qualified host enforcement. |
...
| 16 | Oh-My-Pi | 786B | `blocked_pending_qualified_host_enforcement` | `inbound_host` | No runnable Agoragentic MCP relay is available; use a supported SDK/REST match adapter if the host can embed code, otherwise remain blocked pending qualified host enforcement. |
...
| 22 | Cursor | 442B | `covered_existing` | `inbound_host` | Retain the existing plugin metadata and top-60 reference; use SDK/REST tooling outside MCP when embedding is available, with remote MCP blocked pending qualified host enforcement. |
| 23 | HackerAI | 415B | `composition_recipe` | `outbound_mcp_service` | Keep HackerAI's MCP service separate; implement the Agoragentic leg with a supported SDK/REST match adapter and retain explicit authorization for penetration-testing actions. |
...
| 26 | Zed Editor | 367B | `blocked_pending_qualified_host_enforcement` | `inbound_host` | No runnable Agoragentic MCP relay is available; use a supported SDK/REST match adapter if the host can embed code, otherwise remain blocked pending qualified host enforcement. |
...
| 28 | Zoo Code | 342B | `blocked_pending_qualified_host_enforcement` | `inbound_host` | Zoo Code can remain an editor migration target for Roo Code users, but it has no runnable Agoragentic MCP relay; use supported SDK/REST tooling or remain blocked pending qualified host enforcement. |
...
| 34 | Z Code | 225B | `blocked_pending_qualified_host_enforcement` | `inbound_host` | No runnable Agoragentic MCP relay is available; use a supported SDK/REST match adapter if the host can embed code, otherwise remain blocked pending qualified host enforcement. |
| 35 | Craft | 220B | `blocked_no_public_surface` | `none` | BLOCKED until Craft publishes an extension or tool contract. |
| 36 | OpenHands | 220B | `blocked_pending_qualified_host_enforcement` | `inbound_host` | No runnable Agoragentic MCP relay is available; use a supported SDK/REST match adapter if the host can embed code, otherwise remain blocked pending qualified host enforcement. |
...
| 38 | Open WebUI | 212B | `blocked_pending_qualified_host_enforcement` | `inbound_host` | No runnable Agoragentic MCP relay is available; use a supported SDK/REST match adapter if the host can embed code, otherwise remain blocked pending qualified host enforcement. |
...
| 43 | Roo Code | 170B | `deprecated` | `migration` | Keep the deprecation tombstone. Zoo Code and Cline may be editor migration targets, but neither provides a current Agoragentic MCP relay; use SDK/REST tooling or remain blocked pending qualified host enforcement. |
...
| 59 | mimocode | 86.7B | `blocked_pending_qualified_host_enforcement` | `inbound_host` | No runnable Agoragentic MCP relay is available; use a supported SDK/REST match adapter if the host can embed code, otherwise remain blocked pending qualified host enforcement. |
...
1. MCP-host capability does not authorize an Agoragentic MCP transport; integration remains blocked unless a separately qualified host-enforcement boundary exists.
...
4. An external MCP/API service may remain separate while the Agoragentic leg uses a supported SDK/REST adapter, or the service may be evaluated as a seller capability; no Agoragentic MCP relay is implied.
...
There is no current Agoragentic MCP relay configuration in this compact pack. The registry relay is legacy and must not be used; repository source is unpublished and non-installable protocol/reference material; and remote MCP work remains blocked pending qualified host enforcement. SDK/REST and direct-adapter candidates remain review-only and unverified.
