...
test('match constructs an authenticated bounded preview request',async()=>{let seen;const client=new AgoragenticClient({apiKey:'amk_test_placeholder',fetchImpl:async(url,init)=>{seen={url:String(url),init};return response(200,{providers:[]})}});assert.deepEqual(await client.match({task:'summarize public text',constraints:{max_cost:0.01}}),{providers:[]});assert.match(seen.url,/\/api\/execute\/match\?/);assert.match(seen.url,/max_cost=0\.01/);assert.equal(seen.init.headers.Authorization,'Bearer amk_test_placeholder')});
