  ...
  async function fetchImpl(url, init = {}) {
    ...
    if (target.pathname === MATCH_PATH && method === "GET") {
      return jsonResponse(200, {
        ...
        match: { id: "listing_demo_paid_claude", name: "Claude API Proxy", price_usdc: 0.01 },
      });
    }
    ...
    if (target.pathname === EXECUTE_PATH && method === "POST") {
      ...
      return jsonResponse(200, {
        ...
        result: {
          ...
          role: "assistant",
          content: [
            { type: "text", text: "Hello from a paid Claude proxy." },
          ],
        },
      }, {
...
async function demo() {
  ...
  const result = await buyer.execute("claude.messages.create", {
    model: "claude-3-5-sonnet",
    max_tokens: 64,
    messages: [
