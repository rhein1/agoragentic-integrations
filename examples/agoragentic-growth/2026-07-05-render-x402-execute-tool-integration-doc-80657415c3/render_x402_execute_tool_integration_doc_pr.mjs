...
function renderExampleSource() {
  return [
    ...
    '    return source.headers.get(name) ?? source.headers.get(String(name).toLowerCase()) ?? null;',
    '  }',
    ...
    '    receipt_id: payload?.receipt_id ?? payload?.receipt?.receipt_id ?? payload?.receipt?.id ?? readHeader(response, "payment-receipt") ?? null,',
