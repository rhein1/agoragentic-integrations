#!/usr/bin/env node
// demo — moves no real funds; the pay() callback below returns mock authorization only.
...
async function localX402Fetch(url, options = {}) {
  ...
  while (true) {
    ...
    try {
      ...
      if (cachedPayment) {
        throw createHttpError("Paid request received another HTTP 402; refusing to re-authorize payment or replay cached authorization", {
          status: 402,
          idempotencyKey,
...
async function x402Fetch(url, options = {}) {
  ...
  const paymentRequired = existingMeta.paymentRequired ?? readHeader(response, "payment-required") ?? null;
  const paymentAttempted = existingMeta.paymentAttempted
...
export function buildReceiptChecklist({ response, payload, quoteId, idempotencyKey, paymentAttempted, paymentRequired }) {
  ...
  return {
    ...
    checks: [
      ...
      { item: "receipt_quote_binding", status: quoteId ? (responseQuoteId === quoteId ? "pass" : "fail") : "skip", evidence: quoteId ? `${responseQuoteId || "missing"} vs ${quoteId}` : "quote not used" },
...
export async function createMockMcpServer() {
  const state = {
    ...
    quoteId: "quote_demo_paid_hyperliquid_snapshot",
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (req.method === "GET" && url.pathname === MATCH_PATH) {
      ...
      return sendJson(res, 200, {
        ...
        match: {
          id: "listing_demo_paid_hyperliquid_snapshot",
          transport: "mock-mcp-server",
          tool: "hyperliquid.market_snapshot",
          price_usdc: 0.01,
        },
      });
    }
    ...
    if (req.method === "POST" && url.pathname === EXECUTE_PATH) {
      ...
      return sendJson(res, 200, {
        ...
        result: {
          ...
          tool: "hyperliquid.market_snapshot",
          output: { venue: "hyperliquid", coin: body?.input?.coin ?? "BTC", interval: body?.input?.interval ?? "1m", mark_px: "67234.1", best_bid: "67233.8", best_ask: "67234.4", oracle_px: "67231.9" },
        },
        receipt: {
          id: receiptId,
...
async function demo() {
  ...
  try {
    ...
    const result = await buyer.execute("hyperliquid.market_snapshot", { coin: "BTC", interval: "1m" });
    ...
    console.log(JSON.stringify({
      demo: "x402 execute receipt checklist for hyperliquid data integration",
