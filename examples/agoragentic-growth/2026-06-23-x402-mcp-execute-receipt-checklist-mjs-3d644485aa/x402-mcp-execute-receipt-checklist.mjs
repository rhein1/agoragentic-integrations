...
export class X402McpExecuteReceiptChecklistClient {
  constructor(options = {}) {
  ...
  async previewToolCall(request) {
    assertRequiredString(request?.server, 'server');
  ...
  async executeToolCall(input) {
    ...
    const proposedFingerprint = hashJson(proposedBody);
    ...
    if (existing && existing.quote_id !== input.quoteId) {
      throw new Error(`existing session ${existing.session_id} is bound to quote ${existing.quote_id}, not ${input.quoteId}`);
    }
    if (existing && !requestMatchesState(existing, proposedBody, proposedFingerprint)) {
      throw new Error(`existing session ${existing.session_id} is bound to a different execute request body`);
    }
    if (existing && hasTerminalOrExplicitDemoEvidence(existing)) {
      return this.buildResult(existing, existing.result_snapshot || {});
    }

    const state = existing || createState(input, proposedFingerprint);
    state.request_fingerprint = proposedFingerprint;

    const body = existing?.request_body ? cloneJson(existing.request_body) : proposedBody;
    ...
    try {
      const rawResult = await x402Fetch(`${this.baseUrl}/api/x402/execute`, {
        ...
        pay: async (paymentRequired, requestContext) => {
          ...
          const cachedAuthorization = this.authorizationCache.get(state.session_id);
          if (cachedAuthorization) {
            appendTimeline(state, 'payment_reused', 'reusing cached payment authorization after prior 402');
            return cloneJson(cachedAuthorization);
          }
          ...
          this.authorizationCache.set(state.session_id, cloneJson(authorization));
      ...
      if (!result.response?.ok) {
        ...
        terminal.authorization = redactAuthorization(result.paymentAuthorization || null);
        throw terminal;
      }
      ...
      if (result.paymentAuthorization) {
        this.authorizationCache.set(state.session_id, cloneJson(result.paymentAuthorization));
      ...
      if (error?.authorization) {
        error.authorization = redactAuthorization(error.authorization);
      }
...
async function demo() {
  ...
  let payCalls = 0;
  const client = new X402McpExecuteReceiptChecklistClient({
    apiKey: 'demo_api_key',
