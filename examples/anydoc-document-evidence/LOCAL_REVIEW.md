...
The review profile checks supported packet identity; forced-false authority and pending handoff flags; source identity; output digest; bounded ordered evidence-unit coverage; unit and aggregate hashes; receipt identity and hash/count/coverage/completeness binding; format-bound semantic risk and exact known limitations; the required high-risk decision-use blocker; and inconsistent completeness claims. This is a focused consumer profile, not exhaustive validation of every field in the full adapter schema. Cryptographic consistency is not signature verification, parser authentication, source-exact extraction, semantic correctness, or a completed commercial outcome.

The format/risk check rejects an inconsistent one-field downgrade against the packet's declared format. It does not authenticate that the unsigned declared format came from the pinned parser: coordinated relabeling of the checked format/risk claims can remain internally consistent. Such a result is still only `local_packet_consistency`, with `parser_authenticated`, `semantic_correctness_verified`, and `context_approved` all false. A `complete` result means only that the declared parse envelope has no completeness blockers; the receipt remains pending and no authority is granted.
...
The focused review suite contains 25 provider-free tests, including risk-profile tampering, high-risk blocker removal/duplication, truncation contradictions and descriptor replacement probes. The aggregate package suite currently contains 44 tests, including three real pinned-AnyDoc CSV producer-to-review cases. Native parser conformance and Windows behavior must be assessed from current-head CI, not the local source-subset result. The historical alert 884's most recent PR instance is fixed; current-head CodeQL success remains a separate result that must be checked rather than inferred.
