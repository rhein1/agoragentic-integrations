...
`source_merged` refers to the public protocol source; `host_control_source_merged` refers only to a separate private control-plane source tranche. The two `merged_source_*` fields are source-and-release claims: neither source merge grants activation authority or establishes live protection. The runtime fields remain `unknown` because this public repository has no current deployment probe or authoritative public runtime release evidence. The public local-reference adapter creates a bounded protocol-simulator child only when explicitly invoked. A separate private source tranche defines a default-off path intended only for fixed synthetic USD 0 local-Docker qualification; its public-safe evidence class covers shared-kernel lifecycle, kill, cleanup, and bounded import and is production-ineligible. Source and merge evidence do not establish whether that private path is currently mounted or invoked. Neither source path is evidence of production isolation or live-traffic protection.
