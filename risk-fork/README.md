...
The deterministic suite includes dedicated PostgreSQL contract, independent-process race, crash/ambiguity, revocation, audit, and clean-controller integration tests. Historical pushed-head CI for the source implementation is authoritative only for the exact commit shown on PR #298. Release-candidate CI introduced by PR #360 is likewise evidence only for the exact source SHA checked out by that run; retained build artifacts do not establish production readiness.

PR #298 and its source-only/default-off security follow-up PR #346 are merged. Those source merges granted no deployment, provider-use, spend, live-traffic, publication, or production-activation authority. Those gates remain independently blocked.
...
### Source and activation stages

```yaml
status_scope: merged_source_and_release_evidence
source_merged: true
host_control_source_merged: unknown
merged_source_grants_production_activation_authority: false
merged_source_establishes_live_traffic_protection: false
production_runtime_activated: unknown
live_traffic_protected: unknown
```

`source_merged` refers to the public protocol source. `host_control_source_merged` remains `unknown` in this public record because no immutable public-safe evidence ref/hash is checked in that binds a private release or its interface inventory. The two `merged_source_*` fields describe the evidenced public source merge: it grants no activation authority and does not establish live protection. The runtime fields remain `unknown` because this public repository has no current deployment probe or authoritative public runtime release evidence. The public local-reference adapter creates a bounded protocol-simulator child only when explicitly invoked. That public source path is not evidence of production isolation or live-traffic protection.
...
| **EXTEND** | Tool interception | Adds a fail-closed public host-enforcement bundle and planning seam with source/loopback coverage before real MCP `server/discover`; this public record does not establish a private host-control release or production installation, and live phase coverage remains follow-up work |
...
The `agoragentic.risk-fork.e2b-qualification-evidence.v1` shape is a private-alpha draft contract, and security tranches may deliberately replace finalized receipt fields in place. The external receipt now includes the privilege-separated observer boundary and seven per-birth-control records in addition to the existing network, provider-control, and exact six-decimal cost records. Historical provisional artifacts with `external_observation_receipt: null` remain valid only when every externally derived control is `unknown`; older finalized receipts without the new closed fields fail closed. The core package, generated hosted bundle, and private hosted consumer must therefore be rebuilt and reviewed atomically before that finalized form is consumed.

`applyE2BExternalQualificationObservation()` is the only evidence finalizer. It embeds the exact canonical signed observer receipt—including the signed payload, Ed25519 SPKI, key hash, observation hash, signature, requested limits, bindings, a closed observer boundary, seven birth-control records, network claims, provider-control records, and distinct cost records—rather than retaining only hashes. A verified boundary must be a provider control plane or privileged host supervisor, must bind external evidence, and must prove both child write access and reusable child signing authority false. No birth control can become verified under an unknown or failed boundary. Every later `validateE2BQualificationEvidence()` call for finalized evidence requires the caller-pinned observer verifier, re-verifies the embedded signature, reconstructs the provisional base, and deterministically re-derives every finalized control, cost, and evidence reference. Provider cost cap, derived estimate, aggregate console delta, and finalized per-sandbox actual cost are separate receipt records; a missing finalized actual cost leaves `cost_within_cap=unknown`. A boot-local IPv6 denial or no-route result cannot become verified without separately signed provider IPv6-denial evidence. Missing, changed, mismatched, unsigned, duck-typed, or timeout-ambiguous evidence fails closed as an error or remains `unknown`.
