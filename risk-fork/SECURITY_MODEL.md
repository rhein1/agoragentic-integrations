...
An unknown or untrusted server must be isolated before connecting or accepting content. Merely wrapping the eventual tool invocation is too late. The repository includes a source-only fail-closed host-enforcement bundle plus loopback ordering evidence before `server/discover`. Separate private host-control source is merged for status/preview/stop. Its source defines a default-off, fixed-synthetic, shared-kernel local-Docker qualification path that is production-ineligible. The merged source grants no production execution/interception path. The source contract and tests do not prove hosted enforcement, deployment, or any current live-runtime state; this public repository has no current probe that establishes whether a live enforcement caller exists.
