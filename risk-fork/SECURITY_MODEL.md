...
An unknown or untrusted server must be isolated before connecting or accepting content. Merely wrapping the eventual tool invocation is too late. The repository includes a source-only fail-closed host-enforcement bundle plus loopback ordering evidence before `server/discover`. Separate private host-control source is merged for status/preview/stop. Its local-Docker qualification path is explicitly invoked, private-staging-only, fixed-synthetic, shared-kernel, and production-ineligible. The merged source grants no production execution/interception path. The source contract and tests do not prove hosted enforcement, deployment, or a live-runtime negative; no live enforcement caller is established here.
