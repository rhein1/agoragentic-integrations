...
The local stdio gate accepts only MCP initialization, ping, `tools/list`, `tools/call`, and the client lifecycle notifications `notifications/initialized` and `notifications/cancelled`. It requires a successful validated `initialize` response followed by a fresh successful validated `tools/list` inventory before forwarding `tools/call`; a new initialize or inventory attempt clears the corresponding readiness, so an error or cancellation cannot inherit older authorization. Only one initialize-or-inventory request may be unresolved at a time, including a canceled request awaiting its terminal response or deadline, which prevents out-of-order lifecycle responses from authorizing a newer generation. It rejects resource, prompt, notification-shaped tool-call, and arbitrary method access; rejects all gateway-initiated requests and notifications; requires a closed JSON-RPC result-or-error response shape; rejects any call other than `risk_fork_protect`; reads and hashes the gateway once through a nonblocking open file descriptor, then streams those verified bytes to a child bootstrap instead of reopening the mutable pathname; strips inherited environment authority before spawning the gateway; and fails closed if the gateway advertises zero, two, paginated, differently named, or non-object-input tools.

The credential-pattern defense permits only the standard string or safe-integer `params._meta.progressToken` exception; that value remains scanned, and `progressToken` in any other structure is rejected. Recognized bearer/basic authorization material, Proxy-Authorization text, URL userinfo, credential-shaped property names and two-string credential-header tuples, and other finite recognized secret patterns fail closed in both directions without being logged or relayed. Ordinary policy prose and governance tuples remain available. This scanner is defense in depth, not proof that arbitrary unknown credential formats cannot pass while `tool_input_schema_bound` remains `false`.

At most 16 requests may be active, 16 canceled requests may remain unconfirmed, and 32 total gateway operations may remain unresolved. Each has a 30-second deadline. Gateway stderr is discarded and cumulatively capped at 65,536 bytes; overflow, input or output backpressure, premature gateway-output EOF, or asynchronous pipe failure closes the gate rather than accumulating an unbounded queue or leaking an uncaught process error. A cooperative gateway may exit cleanly after client EOF; an unexpected pre-EOF exit remains a failure. A POSIX process group cannot contain a descendant that creates a new session, and this dependency-free source has no cgroup, PID namespace, Windows Job Object, or equivalent kernel-owned tree boundary. The shipped `serve` command therefore refuses on every platform before reading or starting gateway code. Packet planning and inactive review remain available; activation requires a separately reviewed host supervisor that supplies enforceable descendant containment.
...
This PowerShell command creates disabled review artifacts only. The generated gate command cannot be activated from this source tranche on any platform because `serve` fails closed before reading or starting gateway code until a kernel-owned descendant boundary is integrated.
