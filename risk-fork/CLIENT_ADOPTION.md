...
The credential-pattern defense permits only the standard string or safe-integer `params._meta.progressToken` exception; that value remains scanned, and `progressToken` in any other structure is rejected. Recognized bearer/basic authorization material, Proxy-Authorization text, URL userinfo, credential-shaped property names, and other finite recognized secret patterns fail closed in both directions without being logged or relayed. This scanner is defense in depth, not proof that arbitrary unknown credential formats cannot pass while `tool_input_schema_bound` remains `false`.

At most 16 requests may be active, 16 canceled requests may remain unconfirmed, and 32 total gateway operations may remain unresolved. Each has a 30-second deadline. Gateway stderr is discarded and cumulatively capped at 65,536 bytes; overflow, input or output backpressure, premature gateway-output EOF, or asynchronous pipe failure closes the gate rather than accumulating an unbounded queue or leaking an uncaught process error. A cooperative gateway may exit cleanly after client EOF; an unexpected pre-EOF exit remains a failure. `serve` is deliberately unavailable on Windows because this dependency-free gate does not have an enforceable Windows descendant-process boundary; packet planning and review remain available there, but activation requires a qualified non-Windows host or a future independently reviewed Windows Job Object implementation.
...
This PowerShell command creates disabled review artifacts only. The generated gate command cannot be activated on Windows in this tranche because `serve` fails closed before reading or starting gateway code.
