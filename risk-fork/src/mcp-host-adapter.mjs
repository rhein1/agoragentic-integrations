...
function normalizeTarget(refValue, originValue) {
  const href = requireExternalEndpoint(refValue, 'MCP request.mcp_server_ref');
  const origin = requireExternalEndpoint(originValue, 'MCP request.mcp_server_origin');
...
function assertPlanMatchesRequest(planValue, planRequest, request, maxBytes) {
  try {
    ...
    if (operationInput.network_policy?.mode !== 'blocked') {
      throw new TypeError('MCP phase plan child network policy must remain blocked');
    }
    requireSha256Ref(
...
function cleanImportedEnvelope(request, plan, preparedResult) {
  try {
    ...
    const plannedCandidate = plannedInput.operation?.commit_candidate;
    ...
    const result = boundedCanonicalClone(
    ...
    const evidenceRef = `risk-fork-mcp:${prepared.artifact.artifact_hash.slice(7, 31)}`;
    const envelope = {
...
async function executePhase(record, request, context) {
  ...
  throwIfAborted(context);
  const plan = assertPlanMatchesRequest(
    unresolvedPlan,
    planRequest,
    request,
  ...
  throwIfAborted(context);
  return cleanImportedEnvelope(request, plan, preparedResult);
}
