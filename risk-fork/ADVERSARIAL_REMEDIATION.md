...
| --- | --- | --- |
| E2B | Unconfigured savepoint, fork-create, and execution entrypoints throw `E2B_SECURE_SNAPSHOT_PROFILE_UNAVAILABLE` before verifier, SDK, or provider I/O. The configured strict clean-template profile stages a bounded immutable sanitized manifest export, requires external authority-free verification, requests empty env/IAM/mounts and deny-all SDK network settings at birth, exact-binds bootstrap and runner results, imports results through a bounded deadline-controlled stream, and durably journals/reconciles cleanup while poisoning allocation on unknown absence | Strict offline/mock conformance only; no credentialed provider run or live containment, first-instruction/IPv6 egress, inherited-state exclusion, idle-TTL, latency, or cost qualification |
...
| Clean commit | Production mode requires the exactly branded PostgreSQL authority: server-time serializable transactions lock parent/governance/approval/optional authorization rows, durably record `prepared` then `effect_started`, pass a unique effect fence to the clean callback, atomically finalize result/head/consumption, preserve unresolved ambiguity, allow only exact proven success to finalize during reconciliation, and serialize an append-only audit chain; non-success reconciliation never releases post-effect reservations, and file authorities remain demonstration-only | Real tests cover independent processes/connections against one PostgreSQL server; they do not qualify managed roles/TLS, cluster failover, multi-region operation, backup/restore, retention operations, or live deployment, and no generic exactly-once external-effect claim is made |
...
The remediation suite now includes dedicated PostgreSQL contract, independent-process race, pending-effect reconciliation race, crash/ambiguity, revocation, reconciliation/audit, and production-controller integration regressions. The pending-effect regression holds the original callback open while a verifier reports point-in-time absence and proves that the parent, approval, and one-use authorization remain unavailable, so no second callback can begin. It also includes configured E2B clean-template, immutable-export, exact bootstrap/result binding, streamed-result limit/deadline, cleanup-journal, restart-reconciliation, and fail-closed default regressions. Local validation is checkout-specific; pushed-head CI is authoritative only for the exact commit shown on PR #298 and must be checked there. Green source, mock-provider, and single-server PostgreSQL results do not establish production readiness.
...
2. Close [#302](https://github.com/rhein1/agoragentic-integrations/issues/302): independently review and credentialed-live-qualify the configured clean-template profile with an owner-authorized bounded canary. Prove template provenance; first-instruction IPv4 and IPv6 egress behavior; absence of inherited environment variables, credential files, process tokens, random/nonce state, sockets, and persistent writable mounts; provider lifecycle/destruction semantics; a verified idle-TTL control; and observed latency/cost. The current offline/mock profile is not that evidence.
3. Finish the production boundary for [#303](https://github.com/rhein1/agoragentic-integrations/issues/303): independently review the concrete PostgreSQL implementation, then qualify a managed deployment with least-privilege roles, TLS, migration/retention operations, backup/restore, failover behavior, monitoring, and an operator reconciliation runbook. Current evidence is one-server source/test evidence only.
