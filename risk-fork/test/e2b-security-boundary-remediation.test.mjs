...
test('E2B result transport rejects every wrong exact-job binding', async (t) => {
  const wrongBindings = [
    ...
    ['parent state hash', { parent_state_hash: hash('wrong-parent-state') }],
    ['capsule hash', { capsule_hash: hash('wrong-capsule') }],
    ['fork identity hash', { identity_hash: hash('wrong-identity') }],
    ['provider ref', { provider_ref: 'provider:wrong-e2b' }],
    ['template id hash', { template_id_hash: hash('wrong-template') }],
    ['MCP phase', { mcp_phase: 'initialize' }],
    ['MCP server ref', { mcp_server_ref: 'server:wrong-e2b' }],
    ['tool name', { tool_name: 'wrong_synthetic_tool' }],
    ['effective arguments hash', { effective_arguments_hash: hash('wrong-arguments') }],
    ['network policy hash', { network_policy_hash: hash('wrong-network-policy') }],
    ['operation hash', { operation_hash: hash('wrong-operation') }],
    ['expected schema hash', { expected_result_schema_hash: hash('wrong-schema') }],
