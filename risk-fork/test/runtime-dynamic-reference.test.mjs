...
test('runtime entry: real reference lifecycle accepts a bounded source and verifies cleanup', {skip: process.platform === 'win32' ? 'Windows local storage is fail-closed until ACL proof exists' : false}, async()=>{
...
test('runtime entry: real taint gate refuses an injected instruction and still verifies cleanup', {skip: process.platform === 'win32' ? 'Windows local storage is fail-closed until ACL proof exists' : false}, async()=>{
...
test('runtime entry: actual Risk Fork path rejects the bargain and selects the adequate source', {skip: process.platform === 'win32' ? 'Windows local storage is fail-closed until ACL proof exists' : false}, async()=>{
