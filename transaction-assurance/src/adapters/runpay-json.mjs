...
export function parseRunpayJson(input) {
...
export function snapshotJson(input) {
  if (typeof input === 'string' || Buffer.isBuffer(input)) return parseRunpayJson(input);
...
export function canonicalRunpayJson(value) {
  if (Array.isArray(value)) return `[${value.map(canonicalRunpayJson).join(',')}]`;
  if (value !== null && typeof value === 'object') {
    return `{${Object.keys(value).sort().map((key) => `${JSON.stringify(key)}:${canonicalRunpayJson(value[key])}`).join(',')}}`;
  }
  return JSON.stringify(value);
}
export const digestJson = (value) => `sha256:${createHash('sha256').update(canonicalRunpayJson(value), 'utf8').digest('hex')}`;
